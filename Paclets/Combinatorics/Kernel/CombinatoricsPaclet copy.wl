...
ClearAll[PermutationFromIndex, TupleFromIndex, LehmerCodeFromPermutation,
     PermutationIndex, OrderedTupleIndex, TupleIndex, SubsetIndex, SubsetFromIndex,
     PermutationToTableaux, TableauxToPermutation, DiscreteInverseLessThan,
     DiscreteInverseEqual, RepetendCycleLength, NDigitNumbers]

PermutationFromIndex[index_Integer, length_Integer] :=
    PermutationFromLehmerCode[LehmerCodeFromIndex[index, length]];

LehmerCodeFromIndex[index_Integer, length_Integer] :=
...
TupleFromIndex[index_Integer, len_Integer] :=
...
OrderedTupleFromIndex[index_Integer, len_Integer] :=
...
LehmerCodeIndex[lehmer_List] :=
    Total[lehmer Reverse[Range[0, Length[lehmer] - 1]]!];

LehmerCodeFromPermutation[perm_List] :=
    Flatten[Table[Position[Sort[Drop[perm, n - 1]], perm[[n]]] - 1, {
        n, 1, Length[perm]}]];

OrderedTupleIndex[tuple_List] :=
...
DiscreteInverseLessThan[function_, n_] :=
    NestWhile[#1 + 1&, 1, function < n&, 1, \[Infinity], -1] /; Head[
        function] != Function

DiscreteInverseEqual[function_, n_] :=
    NestWhile[#1 + 1&, 1, function != n&] /; !MatchQ[function, _Function
        ]

Attributes[RepetendCycleLength] = {Listable};

RepetendCycleLength[n_ ? (IntegerQ[#] \[And] # > 0&)] :=
    Block[{q},
        q = Last[First[RealDigits[1 / n]]];
        If[IntegerQ[q],
            q = {}
        ];
        Length[q]
...
NDigitNumbers[n_, base_] :=
    Range[base ^ (n - 1), base ^ (n) - 1]
