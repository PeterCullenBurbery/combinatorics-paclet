...
PeterBurbery`CombinatoricsPaclet`NumberOfTableaux;
...
NumberOfTableaux[{}]:=1;
NumberOfTableaux[s_List]:=With[{transpose=ResourceFunction["ConjugatePartition"][s]},(Total[s]!)/Product[transpose[[col]]-row+s[[row]]-col+1,{row,Length[s]},{col,s[[row]]}]];
NumberOfTableaux[n_Integer]:=Total[Map[NumberOfTableaux,IntegerPartitions[n]]];
